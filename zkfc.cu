...
FrTensor zkFC::operator()(const FrTensor& X) const { // X.size is batch_size * input_size
    ...
    matrixMultiplyOptimized<<<gridSize, blockSize>>>(X.gpu_data, weights.gpu_data, out.gpu_data, batchSize, inputSize, outputSize);
    {
        ...
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("zkFC::operator() matrixMultiplyOptimized launch failed: ") + cudaGetErrorString(err));
        }
        err = cudaDeviceSynchronize();
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("zkFC::operator() matrixMultiplyOptimized sync failed: ") + cudaGetErrorString(err));
        }
    }
    if(has_bias){
        ...
        auto err = cudaGetLastError();
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("zkFC::operator() fcAddBiasKernel launch failed: ") + cudaGetErrorString(err));
        }
        err = cudaDeviceSynchronize();
        if (err != cudaSuccess) {
            throw std::runtime_error(std::string("zkFC::operator() fcAddBiasKernel sync failed: ") + cudaGetErrorString(err));
        }
    }
    return out;
}
