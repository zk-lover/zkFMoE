...
cmd=['./self-attn','attn','opt-input-int-padded.bin','4','1024','./zkllm-workdir/OPT-125m','layer-0','opt-attn-out.bin']
...
with subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True) as p:
    ...
    with open('attn_timestamped.txt','w') as f:
        f.write(f"start\t{start:.6f}\n")
        f.write(f"end\t{end:.6f}\n")
        f.write(f"total\t{end-start:.6f}\n")
        for t,s in lines:
            f.write(f"{t:.6f}\t{s}\n")
    print('WROTE attn_timestamped.txt',file=sys.stderr)
