...
KERNEL void rescaling_kernel(Fr_t* in_ptr, Fr_t* out_ptr, Fr_t* rem_ptr, long scaling_factor, uint N)
{
    ...
    {
        if (scaling_factor == 1) {
            // identity rescaling: copy input directly and zero remainder.  avoid
            // any narrowing conversion, which can corrupt large field values.
            out_ptr[tid] = in_ptr[tid];
            rem_ptr[tid] = {0,0,0,0,0,0,0,0};
        } else {
...
vector<Claim> Rescaling::prove(const FrTensor& X, const FrTensor& X_)
{
    ...
    // for a unit scaling factor the operation is identity and rem should
    // always be zero.  the kernel that constructs `rem_tensor_ptr` uses a
    // narrowing conversion to `long` which can lose information for field
    // elements outside the 64-bit signed range; when working with results of
    // matrix multiplication this happens frequently and produced spurious
    // failures during demo runs.  we therefore treat factor==1 as a special
    // case and avoid the problematic check altogether.
    if (scaling_factor == 1) {
        if (X(u) != X_(u)) {
            throw std::runtime_error("Error: identity rescaling failed");
        }
    } else {
    ...
    // For identity rescaling, remainder is structurally zero and no lookup
    // argument is needed. Skipping tLookup here avoids large-instance
    // instability without changing semantics.
    if (scaling_factor == 1) {
        return {};
    }
